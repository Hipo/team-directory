  ...
  return (
    <div className={"main-sidebar"}>
      <div className={"sidebar-tabs"}>
        <button onClick={handleTeamsTabClick} className={teamsTabClassName}>{"teams"}</button>
        <button onClick={handleProjectsTabClick} className={projectsTabClassName}>{"projects"}</button>
      </div>
      <div className={"sidebar-tab-items"}>
        <ul className={"sidebar-tab-items-container"}>
          {sidebarList.map((item, index)=>(<SidebarRow item={item} key={`sidebar-row-item-${index}`}/>))}
        </ul>
      </div>
    </div>
  )
